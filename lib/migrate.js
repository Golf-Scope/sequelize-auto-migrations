...
const reverseSequelizeColType = function (col, prefix = 'Sequelize.') {
  ...
  switch (attrName) {
    ...
    case DataTypes.ARRAY.key:
      ...
      if (arrayType.includes('VARCHAR')) {
        arrayType = arrayType.replace('VARCHAR', 'STRING');
      }
      
      return prefix + `ARRAY(${arrayType})`;

    case DataTypes.RANGE.key:
      if (attrObj.toSql() === 'tstzrange') {
        return prefix + 'RANGE(Sequelize.DATE)';
      } else {
        // BOOLEAN
        // TIME
        // DATEONLY
...
const parseIndex = function (idx) {
  ...
  if (idx.using && idx.using.toLowerCase() === 'gist') {
    options.using = 'gist';
  } else if (idx.using && idx.using.toLowerCase() === 'gin') {
    options.using = 'gin';
  }
...
const getMigration = function (actions) {
  ...
  for (let _i in actions) {
    ...
    ${JSON.stringify({...action.options, concurrently: true})}
] }`;
          commandsUp.push(res);
